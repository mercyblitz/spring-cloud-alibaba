 ...
 */
public class FeignMetadataResolver
	...
	public FeignMetadataResolver(String currentApplicationName,
			ObjectProvider<Contract> contract) {
	...
	public Set<ServiceRestMetadata> resolveServiceRestMetadata(ServiceBean serviceBean) {
		...
		urls.stream().map(SpringCloudRegistry::getServiceName).forEach(serviceName -> {
	...
	protected RestMethodMetadata resolveMethodRestMetadata(MethodMetadata methodMetadata,
		...
		metadata.setMethod(
				new org.springframework.cloud.alibaba.dubbo.metadata.MethodMetadata(
						feignContractMethod));
